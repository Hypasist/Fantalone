class_name DisCmdGetPushedToDeath
extends DisCmdBase
...
func execute():
	Terminal.add_log(Debug.ALL, Debug.DISPLAY_CMD, "[%s] DisCmdGetPushedToDeath %s" % [object_logic.get_name_id(), object_logic.hex.coords.to_str()])
