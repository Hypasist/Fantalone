...
func setup():
	rpc_config("execute_command", MultiplayerAPI.RPC_MODE_SYNC)
...
func execute_command(cmd, param1=null, param2=null, param3=null, param4=null):
	...
	match cmd:
		command.BROADCAST_LOBBY:
			pass
		command.BROADCAST_START:
			rpc("execute_command", command.START_GAME)
		command.START_GAME:
			mod.Match.start_match()
		command.REQUEST_IDENTIFICATION:
			rpc_id(param1, "execute_command", command.REQUEST_NEW_MEMBER)
		command.REQUEST_NEW_MEMBER:
			var player_name = mod.Database.get_player_name()
			rpc_id(mod.Network.SERVER_ID, "execute_command", command.NEW_MEMBER, player_name)
		command.NEW_MEMBER:
		...
		command.REQUEST_NAME_CHANGE:
			rpc_id(mod.Network.SERVER_ID, "execute_command", command.NAME_CHANGE, param1)
		command.NAME_CHANGE:
			mod.LobbyData.change_player_name_by_network_id(network_id, param1)
			
		command.REQUEST_COLOR_CHANGE:
			rpc_id(mod.Network.SERVER_ID, "execute_command", command.COLOR_CHANGE, param1)
		command.COLOR_CHANGE:
		...
		command.REQUEST_UPDATE_LOBBY:
			rpc_id(mod.Network.SERVER_ID, "execute_command", command.UPDATE_LOBBY)
	...
	if refresh_lobby_commands.has(cmd) and mod.Network.is_server():
		...
		rpc("execute_command", command.UPDATE_LOBBY, update_package)
