...
func _init(param_dictionary).(param_dictionary):
	...
	Terminal.add_log(Debug.ALL, Debug.LOGIC_CMD, "[%s] New LogSubCmdGetPushedAndDie from %s to %s" % [unit.get_name_id(), unit.hex.coords.to_str(), destination_hex.coords.to_str()])
