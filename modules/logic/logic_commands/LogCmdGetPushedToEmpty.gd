...
func _init(unit_, destination_hex_).(unit_):
	...
	Terminal.addLog(unit.get_name_id() + " new LogCmdGetPushedToEmpty from " + unit.hex.coords.to_str() + " to " + destination_hex.coords.to_str())

func execute():
	Terminal.addLog(unit.get_name_id() + " LogCmdGetPushedToEmpty " + unit.hex.coords.to_str())
