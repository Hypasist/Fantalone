...
func consume_mockup(Data):
	consume_tile_mockup(Data)
	var match_ids = []
	for player in Data.LobbyData.get_players():
		match_ids.append(player.match_id)
	consume_unit_mockup(Data, match_ids)
	hide()
...
func consume_tile_mockup(Data):
	var tile_mapping = get_used_ids($Tiles)
	for id in tile_mapping:
		var resource_name = _tiles_by_id[id]
		for coords in tile_mapping[id]:
			Data.ObjectData.create_new_object(resource_name, coords)

func consume_unit_mockup(Data, players):
	...
	for match_id in unit_mapping:
		if players.has(match_id):
			...
			for coords in unit_mapping[match_id]:
				Data.ObjectData.create_new_object(resource_name, coords, match_id)
